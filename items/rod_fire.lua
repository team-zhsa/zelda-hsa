...
function item:on_created()
  ...
  item:set_savegame_variable("possession_rod_fire")
...
function item:shoot()
  ...
  local fire = map:create_custom_entity({
	...
	sol.audio.play_sound("items/rod_fire")
...
function item:on_using()
  ...
  local rod_fire = map:create_custom_entity({
    x = x,
    y = y,
    layer = layer,
    width = 16,
    height = 16,
    direction = direction,
    sprite = "hero/item/rods/rod_fire",
  })
  ...
  sol.timer.start(rod_fire, 10, function()
    rod_fire:set_position(hero:get_position())
    return true
  end)

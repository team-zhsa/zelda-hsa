...
function sensor_cutscene:on_activated()
	if game:get_value("dungeon_2_discovered") ~= true then
		hero:freeze()
		audio_manager:play_music("cutscenes/kaepora_gaebora")
		game:start_dialog("maps.out.forest.owl_1", function(answer)
			if answer == 1 then
				sensor_cutscene:on_activated()
			elseif answer == 2 then
				hero:unfreeze()
				game:set_value("dungeon_2_discovered", true)
				audio_manager:play_music_fade(map, "outside/east_lost_woods")
				game:start_dialog("maps.out.forest.owl_2", function()

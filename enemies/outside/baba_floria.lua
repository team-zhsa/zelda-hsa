...
function enemy:on_created()
  ...
  self:create_sprite("enemies/outside/baba_floria")
...
function enemy:check_hero()
  ...
  if not in_ground then
    sol.timer.start(self, 2000, function()
      if sprite == "enemies/outside/baba_floria" then sprite:set_animation("walking") end
    end)
  end
  if enemy ~= nil then
    if hy < (ey-esy) then
      if sprite == "enemies/outside/baba_floria" then sprite:set_direction(1) end --up
    elseif hy > (ey+esy) then
      if sprite == "enemies/outside/baba_floria" then sprite:set_direction(3) end --down
    else
      if hx < ex then
        if sprite == "enemies/outside/baba_floria" then sprite:set_direction(2) end --left
      else
        if sprite == "enemies/outside/baba_floria" then sprite:set_direction(0) end --right
      end
    end
  end
...
function enemy:on_custom_attack_received(attack, sprite)
  ...
  if attack == "sword" then
    if sprite:get_direction() == 0 then
      ...
      if hero:get_direction() ~= 2 and not in_ground then
        if sprite == "enemies/outside/baba_floria" then sprite:set_animation("hurt") end
        enemy:hurt(1)
        self:remove_life(1)
        self:check_hero()
      end
  ...
  elseif attack == "arrow" then
    if sprite == "enemies/outside/baba_floria" then sprite:set_animation("ground") end
    in_ground = true
    sol.timer.start(self:get_map(), 3000, function()
      in_ground = false
      if sprite == "enemies/outside/baba_floria" then sprite:set_animation("emerging") end
      self:restart()
    end)
  end
end

...
function quest_submenu:show_info_message()
  ...
  elseif item_name == "golden_leaf_counter" then
    local item = item_name and self.game:get_item(item_name) or nil
    if item:get_amount() > 0 then
      dialog_id =  "scripts.menus.pause_inventory.golden_leaf_counter.1" 
    end
  else
...
function quest_submenu:set_cursor_position(row, column)
  ...
  elseif item_name =="golden_leaf_counter" then
    local item = item_name and self.game:get_item(item_name) or nil
    if item:get_amount() > 0 then
      self:set_caption_key("inventory.caption.item.golden_leaf_counter.1")
      self.game:set_custom_command_effect("action", "info")
   end
  else
    local item = item_name and self.game:get_item(item_name) or nil

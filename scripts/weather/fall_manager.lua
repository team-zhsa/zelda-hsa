...
The functions here defined are:
    game:get_fall_mode()
    game:set_fall_mode(fall_mode)
    game:get_world_fall_mode(world)
    game:set_world_fall_mode(world, fall_mode)

fall modes: "fall", "fall_storm", nil (no fall).
--]]
...
function game_meta:get_fall_mode() return current_fall_mode end
function game_meta:set_fall_mode(fall_mode)
  previous_world = current_world
  fall_manager:start_fall_mode(fall_mode)
end
-- Get/set the fall mode for a given world.
function game_meta:get_world_fall_mode(world)
  return world and self:get_value("fall_mode_" .. world) or nil
end
function game_meta:set_world_fall_mode(world, fall_mode)
...
function fall_manager:on_map_changed(map)
  ...
  previous_world = current_world
  current_world = world
  local fall_mode = current_game:get_world_fall_mode(world)
...
function fall_manager:create_leaf(deviation)
  ...
  if current_fall_mode == "fall_storm" then max_num_leaves = max_num_leaves_fall_storm end

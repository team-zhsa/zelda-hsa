...
-- This script requires the multi_event script and the porters meta:
...
local lightning_duration = 400 -- In milliseconds.
local max_lightning_opacity = 180 -- (Opaque = 255).
...
function rain_manager:start_lightnings()
  ...
  local function create_next_lightning()
    ...
    timers["lightning_creation_timer"] = sol.timer.start(game, lightning_delay, function()
      ...
      local dt = 0 -- Time.
      timers["lightning_duration_timer"] = sol.timer.start(game, 5, function()
        dt = dt + 10
